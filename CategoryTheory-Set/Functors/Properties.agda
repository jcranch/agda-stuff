module Functors.Properties where
...
module unitors-iso {ℓ₁ ℓ′₁ ℓ₂ ℓ′₂ : Level} 
    ...
    open Functors.Composition.unitors F
    open Category D
    ...
    ⊙-unitor-l-iso = makeIso₁ ⊙-unitor-l ⊙-unitor-l⁻¹ (λ x → id-l (id (F₀ x))) (λ x → id-l (id (F₀ x)))

    ⊙-unitor-r-iso : Isomorphism₁ ⊙-unitor-r
    ⊙-unitor-r-iso = makeIso₁ ⊙-unitor-r ⊙-unitor-r⁻¹ (λ x → id-l (id (F₀ x))) (λ x → id-l (id (F₀ x)))
...
module associators-iso {ℓ₁ ℓ′₁ ℓ₂ ℓ′₂ ℓ₃ ℓ′₃ ℓ₄ ℓ′₄ : Level} 
  ...
  open Functors.Composition.associators H G F
  ...
  ⊙-associator-iso = makeIso₁ ⊙-associator ⊙-associator⁻¹ (λ x → Category.id-l C₄ (NatTrans.component ⊙-associator x)) (λ x → Category.id-r C₄ (NatTrans.component ⊙-associator⁻¹ x))
